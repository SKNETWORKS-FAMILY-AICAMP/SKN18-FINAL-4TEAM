...
def _ensure_ruff_issues(state: InterviewState) -> List[Dict[str, Any]]:
    ...
    if "ruff_issues" in state and isinstance(state["ruff_issues"], list):
        return state["ruff_issues"]
...
def collaboration_eval_agent(state: InterviewState) -> InterviewState:

...
def _neo4j_recommend_problem_ids(user_id: str, limit: int, difficulties: list) -> list:
    query = (
        ...
        "  WITH s, coalesce(sup.weight, e.weight, 1.0) AS weight "
        "  MATCH (s)<-[:USES_ALGO]-(p:Problem) "
        "  RETURN p AS problem, weight AS weight "
        "  UNION "
        "  WITH u, sim_factor "
        "  MATCH (u)-[g:SHOWED_GAP]->(s2:AlgoSkill) "
        "  WITH s2 AS s, coalesce(g.weight, 1.0) AS weight "
        "  MATCH (s)<-[:USES_ALGO]-(p:Problem) "
        "  RETURN p AS problem, weight AS weight "
        "  UNION "
        "  WITH u, sim_factor "
        "  MATCH (u)-[sim:SIMILAR]->(u2:User)-[g2:SHOWED_GAP]->(s3:AlgoSkill) "
        "  WITH s3 AS s, coalesce(g2.weight, 1.0) * coalesce(sim.score, 1.0) * sim_factor AS weight "
        "  MATCH (s)<-[:USES_ALGO]-(p:Problem) "
        "  RETURN p AS problem, weight AS weight "
        "  UNION "
        "  WITH u, sim_factor "
        "  MATCH (u)-[g3:SHOWED_GAP]->(s4:AlgoSkill)-[rel:RELATED]->(s5:AlgoSkill) "
        "  WITH s5 AS s, coalesce(g3.weight, 1.0) * coalesce(rel.score, 0.5) AS weight "
        "  MATCH (s)<-[:USES_ALGO]-(p:Problem) "
        "  RETURN p AS problem, weight AS weight "
        "  UNION "
        "  WITH u, sim_factor "
        "  MATCH (u)-[sim:SIMILAR]->(u2:User)-[sol:SOLVED]->(p:Problem) "
        "  RETURN p AS problem, coalesce(sol.score, 1.0) * coalesce(sim.score, 1.0) * sim_factor AS weight "
        "} "
        "MATCH (problem)-[:HAS_DIFFICULTY]->(d:Difficulty) "
        "WHERE d.level IN $difficulties "
        "WITH problem, sum(weight) AS score "
        "RETURN problem.problem_id AS problem_id "
        "ORDER BY score DESC "
        "LIMIT $limit"
    )
    payload = {

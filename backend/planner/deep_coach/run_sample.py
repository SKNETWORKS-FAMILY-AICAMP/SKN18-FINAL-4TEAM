...
# 샘플 입력
SAMPLE_VIDEO_SUMMARY = """주어진 강의에서는 파이썬을 사용하여 깊이 우선 탐색(DFS) 방식으로 트리를 출력하는 방법에 대해 설명한다. 깊이 우선 탐색은 왼쪽 자식 노드를 먼저 방문한 후, 루트 노드를 출력하고, 마지막으로 오른쪽 자식 노드를 방문하는 방식으로 진행된다. 이 과정에서 이진 트리의 경우 출력 결과는 알파벳 순서로 나타난다. 강의에서는 재귀 함수를 사용하여 트리를 출력하는 `dfprint` 함수를 정의한다. 이 함수는 먼저 트리가 비어 있는지 확인하고, 왼쪽 자식 노드가 존재하면 재귀적으로 호출하여 출력한 후, 루트 노드를 출력하고, 오른쪽 자식 노드가 존재하면 다시 재귀적으로 호출하여 출력한다. 이 방식으로 트리를 깊이 우선으로 탐색하여 출력할 수 있다. - 깊이 우선 탐색(DFS) 방식으로 트리 출력 - 왼쪽 자식 → 루트 → 오른쪽 자식 순서 - 이진 트리의 경우 알파벳 순서로 출력 - 재귀 함수를 사용하여 구현 - 트리가 비어 있는지 확인 후 출력 진행 [tags] DFS [lang] python
""".strip()
